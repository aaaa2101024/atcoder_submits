...
void bfs(vector<string> &s, vector<vector<char>> &ans, vector<vector<int>> &test, int h, int w, int i, int j)
{
    ...
    rep(_, n)
    {
        ...
        if (ran(h, w, ii, jj) && test[ii][jj] == INF)
        {
            // if (s[ii][jj] == 'E')
            // {
    ...
    while (!que.empty())
    {
        ...
        while (!temp_que.empty())
        {
            ...
            rep(_, n)
            {
                ...
                if (ran(h, w, ii, jj) && test[ii][jj] > path && ch[ii][jj] == false)
                {
                    // if (s[ii][jj] == 'E')
                    // {
                    //     bac(s, ans, test, h, w, ii, jj, path, n);
                    ...
                    if (s[ii][jj] != '#')
                    {
                        test[ii][jj] = min(path,test[ii][jj]);
                        que.push({ii, jj});
                        ch[ii][jj] = true;
                    }
                }
    ...
    rep(i,h)
    {
        rep(j,w)
        {
            if(ans[i][j] != 'E' && ans[i][j] != '#')
            bac(s,ans,test,h,w,i,j,path,n);
        }
    }
}
...
int main()
{
    ...
    vector<vector<int>> test(h, vector<int>(w, INF));
    rep(i, h)
    {
        rep(j, w)
        {
